...
if (init.result?.serverInfo?.name !== 'pharos-payroll-batch') { console.error('FAIL: bad serverInfo'); process.exit(1); }

const list = await call('tools/list');
const tools = list.result?.tools ?? [];
if (tools.length < 4) { console.error('FAIL: expected 4 tools, got', tools.length); process.exit(1); }
...
console.log('PASS: pharos-payroll-batch — handshake ok, ' + tools.length + ' tools, schema validation ok');
child.kill();
process.exit(0);
