...
if (init.result?.serverInfo?.name !== 'pharos-chain-sentinel') { console.error('FAIL: bad serverInfo'); process.exit(1); }

const list = await call('tools/list');
const tools = list.result?.tools ?? [];
if (tools.length < 6) { console.error('FAIL: expected 6 tools, got', tools.length); process.exit(1); }
...
console.log('PASS: pharos-chain-sentinel — handshake ok, ' + tools.length + ' tools, schema validation ok');
child.kill();
process.exit(0);
