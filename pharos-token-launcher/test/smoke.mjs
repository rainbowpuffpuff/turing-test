...
if (init.result?.serverInfo?.name !== 'pharos-token-launcher') { console.error('FAIL: bad serverInfo'); process.exit(1); }

const list = await call('tools/list');
const tools = list.result?.tools ?? [];
if (tools.length < 8) { console.error('FAIL: expected 8 tools, got', tools.length); process.exit(1); }
...
console.log('PASS: pharos-token-launcher — handshake ok, ' + tools.length + ' tools, schema validation ok');
child.kill();
process.exit(0);
