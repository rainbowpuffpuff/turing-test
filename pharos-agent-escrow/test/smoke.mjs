...
if (init.result?.serverInfo?.name !== 'pharos-agent-escrow') { console.error('FAIL: bad serverInfo'); process.exit(1); }

const list = await call('tools/list');
const tools = list.result?.tools ?? [];
if (tools.length < 10) { console.error('FAIL: expected 10 tools, got', tools.length); process.exit(1); }
...
console.log('PASS: pharos-agent-escrow — handshake ok, ' + tools.length + ' tools, schema validation ok');
child.kill();
process.exit(0);
